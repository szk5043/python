...
class Myclient():
    ...
    def start(self):
        ...
        while True:
            ...
            login_info = ('%s:%s' % (name,passwd))
            self.client.sendall(login_info.encode())      #发送用户密码用户
    ...
    def get(self,command):
        ...
        status_code = self.client.recv(1024).decode()    #接收状态码
        if status_code == '201':
            filename = command.split()[1]

            if os.path.isfile(filename):
                # 文件名存在，判断是否续传
                revice_size = os.stat(filename).st_atime       #文件已接收大小
